...
def query_base(request):
    ...
    articolo_periodo = Articolo.objects.filter(data__range=(datetime.date(2023, 1, 1), datetime.date(2023, 12, 31)))
    
    #10 Gli articoli scritti da giornalisti nati prima del 1980
    giornalisti_nati = Giornalista.objects.filter(anno_di_nascita__lt=datetime.date(1280, 1, 1))
    ...
    #nota per poter mdoficiare la data di un articolo togliere la proprietà auto_now=True al field data nel model 
    #poi dare i comandi makemigrations e migrare per applicare le modifche al database
    ...
    # ... scrivi quali articoli vengono esclusi
